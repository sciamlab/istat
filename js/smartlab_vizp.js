...
var geojson;
var dataset;
var dataset_file = urlParams['data'];
var color_prop;
var height_prop;
...
//                        initDatGUI();
      ...
      function addGeoObject() {
          ...
          for (var i = 0 ; i < geojson.features.length ; i++) {
			  ...
			  var provincia_id = geoFeature.properties.id_provincia;
              ...
              for (var ii = 0 ; ii < dataset.data.length ; ii++) {
                  if(dataset.data[ii].id_regione==provincia_id){
                      current_color = dataset.data[ii][color_prop];
                      current_height = dataset.data[ii][height_prop];
                      break;
                    }
              }
              console.log('['+provincia_id+'] PROVINCIA: ' + geoFeature.properties.nome_provincia + ' color_value: ' + current_color + ' height_value: ' + current_height);
...
// OLD ONE		  var shape3d = meshes[i].extrude({amount: Math.round(extrude), bevelEnabled: false});
              ...
              toAdd.name = geojson.features[i].properties.id_provincia;
        ...
        function tick(){
			...
			if ( intersects.length > 0 ) {						
                ...
                for (var i = 0 ; i < dataset.data.length ; i++) {
                    ...
                    if(region.id_provincia == this.INTERSECTED.name){
                        this.hover_name = region.nome_provincia;
           ...
           if(this.INTERSECTED) {
					$('#footer-text').html('<b>'+this.hover_name+'</b> - '+this.height_label+': '+this.hover_height+this.height_unit+' '+this.color_label+': '+this.hover_color+this.color_unit);
				} else {
					$('#footer-text').html("");
				}
		  onFrame(tick);
